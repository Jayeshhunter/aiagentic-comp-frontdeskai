...
WORKER_CONFIGS = {
    "hr": {
        "system_prompt": (
            ...
            "- Employee wants to CHECK leave balance → call get_leave_balance (this app's own "
            "records). Only if it says the employee is not found are they an HR-system "
            "employee: then call get_leave_balance_from_hr_system (reads PostgreSQL via MCP).\n"
...
_MANAGER_SYSTEM = (
    ...
    "If it says they are an employee of this app, stop: you cannot decide their leave. Tell "
    "them the request is filed and waits for their own manager, quoting the request number "
    "from the HR worker's response if it has one.\n"
